...
W1 <- as.matrix(read.xlsx("W_normalizada.xlsx", sheet = 1, colNames = TRUE, rowNames = TRUE))
      ...
      for (r in 1:length(iR)) {
        for (c in 1:length(iC)) {
          ...
          if (length(g_row) && length(g_col) && g_row == g_col) {
            sub_r_ids <- ids_r[iR]
            sub_c_ids <- ids_c[iC]
            rows_in_g <- which(sub_r_ids %in% groups[[g_row]])
            cols_in_g <- which(sub_c_ids %in% groups[[g_col]])
            Sg <- sum(Wkl_original[rows_in_g, cols_in_g])
            if (abs(Sg) > eps) {
          ...
          if (!length(g_row) && length(g_col)) {
            sub_r_ids <- ids_r[iR]
            RR <- which(sub_r_ids %in% groups[[g_col]])
          ...
          if (length(g_row) && !length(g_col)) {
            sub_c_ids <- ids_c[iC]
            CC <- which(sub_c_ids %in% groups[[g_row]])
